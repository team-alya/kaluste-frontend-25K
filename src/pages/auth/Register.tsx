...
export default function Register() {
 ...
 const handleRegister = (e: React.FormEvent) => {
  ...
  fetch(import.meta.env.VITE_BACKEND_URL + "/api/admin/register", {
      ...
      if (!response.ok) {
        return response.json().then((errorData) => {
          if (errorData.errors) {
            ...
            setErrors({ general: "Rekisteröinti epäonnistui. Yritä uudelleen." });
          }
        });
      }
    ...
    .then((data) => {
      if (data) {
        setSuccessMessage("Rekisteröinti onnistui! Ohjataan takaisin admin-näkymään...");
        setTimeout(() => {
          navigate("/admin");
        }, 2000);
      }
  ...
  return (
    <div className="mt-10 flex justify-center">
      <form onSubmit={handleRegister} className="space-y-7 w-full max-w-md flex flex-col items-center">
        ...
        <div className="text-center">
          ...
          {successMessage && <p className="text-emerald-700 text-xs mb-3">{successMessage}</p>}
        </div>
      </form>
    </div>
  );
